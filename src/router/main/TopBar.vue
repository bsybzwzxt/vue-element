<template>
    <header class="main-header">
        <div class="header-menu">
            <div>
                <i class="fa" :class="$state.layout.sidebarCollapsed ? 'fa-arrow-right' : 'fa-arrow-left'" @click="$state.layout.sidebarCollapsed = !$state.layout.sidebarCollapsed"></i>
            ...
            <div>
                ...
                <el-popover v-model="notice.visible" popper-class="header-menu-notice" @show="noticeShow">
                    <div v-if="notice.data.length > 0">
                        <h3>
                            通知
                            <router-link v-if="notice.data.length > 0" to="/main/personal/notice/table">
                                查看更多<i class="fa fa-chevron-right"></i>
                            </router-link>
                        </h3>
                        <ul>
                            <li v-for="item in notice.data" :key="item.key">
                                <p v-if="item.status === 'new'" class="red">
                                    new~
                                </p>
                                {{ item.title }}
                                <el-button v-if="item.type === 'finish'" type="text" size="mini" @click="download(item)">
                                    点击下载
                                </el-button>
                                <span>
                                    {{ item.date }}
