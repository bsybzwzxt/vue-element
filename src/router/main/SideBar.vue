<template>
    <aside class="main-aside">
        ...
        <div class="scroll-layer">
            <el-menu v-scroll :default-active="defaultActive" :unique-opened="true" :collapse="collapsed" :router="true">
                <template v-for="item in routeList" v-if="!item.access || $state.user.access[item.access]">
                    <el-submenu v-if="item.children && item.children.length > 0" :key="item.index" :index="item.index">
                        ...
                        <template v-for="child in item.children" v-if="!child.access || $state.user.access[child.access]">
                            <el-menu-item :key="child.index" :route="{path: child.path}" :index="child.index">
                    ...
                    <el-menu-item v-if="!item.children || item.children.length === 0" :key="item.index" :route="{path: item.path}" :index="item.index">
                        <i class="fa fa-lg fa-fw" :class="item.icon"></i>
                        <span>
                            {{ item.name }}
                        </span>
